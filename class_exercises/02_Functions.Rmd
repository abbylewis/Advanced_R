...
By this point, you are probably already very familiar with using functions that other people have written. Being able to write your own functions will help you reduce duplication in your code and isolate separate components of your data analysis workflow. This unit provides an in-depth exploration of how functions work in R.
...
```{r}
data <- c(1, 5, 2, 6, 8)
#STEP 1: First, calculate the standard deviation of these data outside of a function

#STEP 2: Now, paste that code in the body of the function. Make sure you use the argument name `x` instead of data
stdev <- function (x) {
  # BODY
}
...
While you almost always create a function and then bind it to a name, the binding step is not compulsory. If you choose not to give a function a name, you get an __anonymous function__. This is useful when it's not worth the effort to come up with a name:

```{r, eval = FALSE}
n_unique <- function(x){ #named function
  length(unique(x))
}
# lapply() applies a function over every column of a dataset. 
# We will talk about this more during future units
# Here is an example of an lapply call using a named function
lapply(mtcars, n_unique)

# This could also be written as an anonymous function
lapply(mtcars, function(x) length(unique(x)))
```

Functions can also go in a list!
...
In R, you'll often see functions called __closures__. This name reflects the fact that R functions capture, or enclose, their environments. We will talk more about environments in future units
...
2. What function allows you to tell if an object is a function? What function allows you to tell if a function is a primitive function? Test using the function `lapply()`
