...
In this tutorial, I will also give a little crash course on creating maps in R and creating interactive visualizations with plotly. For more mapping resources, [this](https://r-spatial.org/r/2018/10/25/ggplot2-sf.html) is my favorite reference.
...
feederwatch %>%
  ...
  summarize(total = sum(how_many, na.rm = T)) %>%
  ungroup() %>%
  top_n(10, total) %>% #Lets only look at the top 10 most abundant birds 
...
ui <- fluidPage(
  #Add title
  titlePanel("Plot of total abundance"),
...
What if we want users to be able to control how many birds show up? We can add an interactive element
...
ui <- fluidPage(
  ...
  #Add slider
  sliderInput("n", "Number to plot", min = 3, max = 30, value = 10),
...
To incorporate this into a shiny app, I am using plotlyOutput() in the UI, and renderPlotly() in the server.
...
You know the drill- just paste into our existing shiny app framework! Only the parts that are dynamic need to be in the server—for the rest (e.g., base maps), it is sufficient we that created these above (i.e., they are in our global environment)
...
Customize this UI to include different text and a different selection interface. The selection interface won't do anything yet, but that's okay! This is just practice.
...
INTERMEDIATE: I'm curious how the relative prevalence of different species changes across months of the year. Can you modify this code to allow users to select what species they want on the plot? I've done the data wrangling for you in the code chunk below—you just need to fill in the shiny app details.
...
server = function(input, output, session) {
  output$plot <- renderPlot({
    ...
    percent_per_month %>%
      filter(american_english_name %in% XXX ) #(fill in this line to respond to the UI)
